...
use crate::model::fe_micro_service_model::{FeMicroServiceCreateReq, FeMicroServiceQuery, FeMicroServiceVo, MicroServiceStatus, MicroServiceUpdateReq};
use crate::service::fe_micro_service_impl::{FeMicroServiceImpl};

pub fn config(cfg: &mut web::ServiceConfig) {
    cfg.service(web::scope("/api/system/fe-micro-service")
...
pub async fn list_used_services(
    ...
    Ok(
        LR::of(
            micro_service.list_with_status(app_id, "open").await?.into_iter().map(|ms| ms.into()).collect()
        )
    )
}
...
pub async fn create_micro_service(
    ...
    Ok(
        LR::of(
                micro_app_service.create(&req).await?.into()
        )
    )
}
...
pub async fn update_micro_service(
    ...
    Ok(
        LR::of(
            micro_app_service.update(&req).await?.into()
        )
    )
}
