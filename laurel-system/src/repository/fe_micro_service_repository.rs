...
impl FeMicroServiceRepository {
    ...
    pub async fn find_service(&self, service_id: &str) -> Running<Option<FeMicroService>> {
    ...
    ) -> Running<Option<FeMicroServiceModel::FeMicroService>> {
        let mut conn = self.pool.get().await?;
        let app = conn
    ...
    ) -> Running<FeMicroServiceModel::FeMicroService> {
        let mut conn = self.pool.get().await?;
        let saved_app = conn
