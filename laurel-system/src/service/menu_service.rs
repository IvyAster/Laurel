...
use crate::model::menu_model::{InsertAbleMenu, Menu, MenuCreateReq, MenuQueryReq, MenuUpdateReq, UpdatableMenu};
...
pub struct MenuService{
    ...
    pub id_api: Arc<IdApi>,
}

impl MenuService{
    pub async fn list_menus(&self, req: &MenuQueryReq) -> Running<Vec<Menu>>{
        let query = req.into();
    ...
    pub async fn list_used_menus(&self, app_id: &str) -> Running<Vec<Menu>>{
        self.menu_repository.list_menus_recursive(app_id).await
    }

    pub async fn page_menus(&self, req: &MenuQueryReq, page: u32, size: u32) -> Running<Pagination<Menu>>{
        let query = req.into();
        self.menu_repository.page_menus(&query, page, size).await
    }

    pub async fn create_menu(&self, req: &MenuCreateReq) -> Running<Menu>{
        ...
        let insertable_menu = InsertAbleMenu{
            ...
            weight: req.weight,
            parent_id: if let Some(x) = &req.parent_id && !x.is_empty() { x.as_str() } else { id.as_str() },
            authority: req.authority.clone(),
            menu_status: if let Some(x) = &req.menu_status && !x.is_empty() { x.as_str() } else { "open" },
            cts: &now,
            uts: &now,
        };
    ...
    pub async fn update_menu(&self, req: MenuUpdateReq) -> Running<Menu>{
        ...
        match &req.parent_id{
            Some(parent_id) => {
                if !(parent_id == &req.menu_id){
                    if self.menu_repository.check_has_child(req.menu_id.as_str(), parent_id.as_str()).await?{
        ...
        Ok(
            self.menu_repository.update_menu(req.menu_id.as_str(), &updatable).await?.expect("当前菜单不存在, 更新失败")
        )
    }
}