use crate::model::menu::{InsertAbleMenu, Menu, MenuQuery, UpdatableMenu};
...
use laurel_pg::{AsyncDsl, DbPool, IntRow, StringRow};
...
impl MenuRepository {
    ...
    pub async fn find_menu(&self, menu_id: &str) -> repository::Result<Option<Menu>> {
    ...
    pub async fn list_menus<'a>(&self, query: &MenuQuery<'a>) -> repository::Result<Vec<Menu>> {
        let mut conn = self.pool.get().await?;
    ...
    pub async fn page_menus<'a>(
        ...
        page: u32,
        size: u32,
    ) -> repository::Result<Pagination<Menu>> {
        let mut conn = self.pool.get().await?;
    ...
    pub async fn list_menus_recursive(&self, app_id: &str) -> repository::Result<Vec<Menu>> {
        let mut conn = self.pool.get().await?;
    ...
    pub async fn check_has_child(&self, menu_id: &str, child_id: &str) -> repository::Result<bool> {
        let mut conn = self.pool.get().await?;
        let query = diesel::sql_query(
            r#"
            WITH RECURSIVE temp_menu AS(
    ...
    pub async fn list_child_menu_ids(&self, menu_id: &str) -> repository::Result<Vec<String>> {
        let mut conn = self.pool.get().await?;
        let query = diesel::sql_query(
            r#"
            WITH RECURSIVE temp_menu AS(
    ...
    pub async fn check_has_parent(&self, menu_id: &str, parent_id: &str) -> repository::Result<bool> {
        let mut conn = self.pool.get().await?;
        let query = diesel::sql_query(
            r#"
            WITH RECURSIVE temp_menu AS(
    ...
    pub async fn list_parent_menu_ids(&self, menu_id: &str) -> repository::Result<Vec<String>> {
        let mut conn = self.pool.get().await?;
        let query = diesel::sql_query(
            r#"
            WITH RECURSIVE temp_menu AS(
    ...
    pub async fn save_menu<'a>(&self, insertable: &InsertAbleMenu<'a>) -> repository::Result<Menu> {
        let mut conn = self.pool.get().await?;
        let menu = conn
