...
impl DictRepository {
    ...
    pub async fn count_dict(&self, dict_id: &str) -> repository::Result<i64> {
    ...
    pub async fn find_dict(&self, id: i64) -> repository::Result<Option<Dict>> {
        let mut conn = self.pool.get().await?;
    ...
    pub async fn page_dict<'a>(
        ...
        page: u32,
        size: u32,
    ) -> repository::Result<Pagination<Dict>> {
        let mut conn = self.pool.get().await?;
        let total = AsyncDsl::get_result::<i64>(
    ...
    pub async fn update(&self, id: i64, updatable: &UpdatableDict) -> repository::Result<Option<Dict>> {
        let mut conn = self.pool.get().await?;
        let dict = conn
    ...
    pub async fn save<'a>(&self, insertable: &InsertableDict<'a>) -> repository::Result<Dict> {
        let mut conn = self.pool.get().await?;
        let dict = conn
    ...
    pub async fn delete(&self, id: i64) -> repository::Result<bool> {
        let mut conn = self.pool.get().await?;
        let result = conn
    ...
    pub async fn count_value(&self, dict_id: &str, value_id: &str) -> repository::Result<i64> {
        let mut conn = self.pool.get().await?;
        let total = AsyncDsl::get_result::<i64>(
    ...
    pub async fn find_value_by_id(&self, id: i64) -> repository::Result<Option<DictValue>> {
        let mut conn = self.pool.get().await?;
    ...
    pub async fn find_value(&self, dict_id: &str, value_id: &str) -> repository::Result<Option<DictValue>> {
        let mut conn = self.pool.get().await?;
        let dict_value = AsyncDsl::first(
            DictValueDsl::dict_value
                .filter(DictValueDsl::dict_id.eq(dict_id))
    ...
    pub async fn page_dict_value<'a>(
        ...
        page: u32,
        size: u32,
    ) -> repository::Result<Pagination<DictValue>> {
        let mut conn = self.pool.get().await?;
        let total = AsyncDsl::get_result::<i64>(
    ...
    ) -> repository::Result<Option<DictValue>> {
        let mut conn = self.pool.get().await?;
        let dict_value = conn
    ...
    pub async fn save_value<'a>(&self, insertable: &InsertableDictValue<'a>) -> repository::Result<DictValue> {
        let mut conn = self.pool.get().await?;
        let dict_value = conn
    ...
    pub async fn delete_value(&self, id: i64) -> repository::Result<bool> {
        let mut conn = self.pool.get().await?;
        let result = conn
            .transaction::<bool, anyhow::Error, _>(|mut tx| {
                Box::pin(async move {
