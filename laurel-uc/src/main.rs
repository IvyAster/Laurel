...
use laurel_actix::config::{load_config, ServerConfig};
...
async fn main() -> std::io::Result<()> {
    ...
    let id_api = IdApi::builder()
        .client(Arc::new(reqwest_middleware::ClientBuilder::new(reqwest::ClientBuilder::new().build().unwrap()).with(RequestLoggingMiddleware).build()))
...
async fn setup() -> (UcAppConfig, DbPool, Redis) {
    let app_config: UcAppConfig = load_config::<UcAppConfig>(None, None).expect("failed to load config");//.try_deserialize().unwrap();//.expect("transfer config failed");
