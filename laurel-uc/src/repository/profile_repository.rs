...
impl ProfileRepository {
    ...
    pub async fn list_with_keys(&self, account_id: &str, keys: &Vec<String>) -> Running<Vec<Profile>> {
        let mut conn = self.pool.get().await?;
        let profiles = ProfileDsl::profile
            .filter(ProfileDsl::account_id.eq(account_id))
    ...
    pub async fn update(&self, account_id: &str, key: &str, value: &str) -> Running<usize>{
        let mut conn = self.pool.get().await?;
        let size = conn.transaction::<usize, anyhow::Error, _>(|mut tx_conn| {
            Box::pin(
                async move{
                    let size = diesel::update(ProfileDsl::profile.filter(ProfileDsl::account_id.eq(account_id)).filter(ProfileDsl::profile_key.eq(key)))
    ...
    pub async fn save(&self, profiles: Vec<(String, String, Option<String>)>)-> Running<usize>{
        ...
        let inserts: Vec<InsertAbleProfile> = profiles.iter().map(|p| InsertAbleProfile{
