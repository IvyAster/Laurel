...
impl Passports {
    ...
    /// 生成秘钥
    pub fn password(account_id: &str, password: &str, salt: &str) -> Running<String> {
        let bytes = Self::strings_to_bytes(account_id, password, salt)?;
    ...
    /// 账户信息组合成字节数组
    fn strings_to_bytes(account_id: &str, password: &str, salt: &str) -> Running<Vec<u8>> {
        let b1 = account_id.encode_utf16().flat_map(|c| c.to_le_bytes()).collect::<Vec<u8>>();
        let b2 = password.encode_utf16().flat_map(|c| c.to_le_bytes()).collect::<Vec<u8>>();
...
mod tests {
    ...
    fn test_password_consistency() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result2 = Passports::password(account_id, password, salt.as_str())?;
